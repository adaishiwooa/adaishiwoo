...
int DemoProfilePositionMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode, long targetPosition)
{
	...
	{
		    /*if(VCS_GetPositionIs(p_DeviceHandle, p_usNodeId, pPositionIs , &p_rlErrorCode) == 0)
			{
				LogError("VCS_GetPositionIs", lResult, p_rlErrorCode);
				lResult = MMC_FAILED;
			}
			printf("previously the position was at %d", *pPositionIs);
*/
...
void eposCallback(const std_msgs::Int32::ConstPtr& msg)
{
    unsigned int lErrorCode = 0;
	int i = 0;
	int lResult =1;
	long targetPositioninputangle = msg->data;
	long targetPositioninput = targetPositioninputangle*78;
	...
	thetimethisloop=(stop-start);
	thetime=start;
	
	printf("now the position is at %ld\n",targetPositioninputangle);
	printf("time is %lf\n",thetime);
	printf("time for move the motor is %lf\n",thetimethisloop);
	sleep(0.1);
...
*/
	...
	ros::Subscriber sub = n.subscribe("epostip", 1, eposCallback);
